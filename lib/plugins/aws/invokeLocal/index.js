...
class AwsInvokeLocal {
  ...
  checkDockerDaemonStatus() {
    return spawn('docker', ['version']).then(({ code }) => {
      if (code) {
        throw Error('Please start the Docker daemon to use the invoke local Docker integration.');
      }
    });
  }

  checkDockerImage(imageName) {
    return spawn('docker', ['images', '-q', imageName]).then(({ stdoutBuffer }) => {
  ...
  pullDockerImage() {
    ...
    return spawn('docker', ['pull', '--disable-content-trust=false', `lambci/lambda:${runtime}`]);
  }
        ...
        layer => {
          ...
          return dirExists(layerContentsPath)
            .then(exists => {
              ...
              return dirExists(layerContentsCachePath);
            })
            .then(exists => {
              if (!exists) {
      ...
      .then(exists => {
        if (exists) {
          return fs.readFileAsync(dockerfileCachePath).then(contents => {
            return contents.toString() === dockerfile && this.checkDockerImage(imageName);
          });
        }
        return false;
      })
      .then(cacheEquals => {
        ...
        return BbPromise.all([
          ...
          .then(() => {
            ...
            return BbPromise.all([
              fs.writeFileAsync(dockerfilePath, dockerfile),
              fs.writeFileAsync(dockerfileCachePath, dockerfile),
            ]);
          })
          .then(() => {
            ...
            return spawn('docker', [
              ...
              if (code) {
                throw Error(`Failed to build docker image (exit code ${code})`);
              }
              return imageName;
            });
          });
  ...
  invokeLocalDocker() {
    ...
    return this.ensurePackage().then(() =>
      BbPromise.all([
        ...
        return docker.then(({ code }) => {
          if (code) {
            throw Error(`Failed to build docker image (exit code ${code})`);
          }
          return BbPromise.resolve(imageName);
        });
      })
    );
  }
  ...
  invokeLocalPython(runtime, handlerPath, handlerName, event, context) {
    ...
    return new BbPromise((resolve, reject) => {
      ...
      python.child.on('close', () => resolve());
      let isRejected = false;
      python.child.on('error', error => {
        isRejected = true;
        reject(error);
      });
    ...
    return new BbPromise((resolve, reject) =>
      fs.statAsync(artifactPath).then(
        () => {
          ...
          java.child.on('close', () => resolve());
          let isRejected = false;
          java.child.on('error', error => {
            isRejected = true;
            reject(error);
          });
          ...
          process.nextTick(() => {
            if (isRejected) return; // Runtime not available
            java.child.stdin.write(input);
            java.child.stdin.end();
          });
        },
        () => {
  ...
  invokeLocalRuby(runtime, handlerPath, handlerName, event, context) {
    ...
    return new BbPromise((resolve, reject) => {
      ...
      ruby.child.on('close', () => resolve());
      let isRejected = false;
      ruby.child.on('error', error => {
        isRejected = true;
        reject(error);
      });
      ...
      process.nextTick(() => {
        if (isRejected) return; // Runtime not available
        ruby.child.stdin.write(input);
        ruby.child.stdin.end();
      });
    });
  }
